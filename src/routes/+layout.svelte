...
<header class="m-5">
	<nav class="flex justify-between items-center">
		<a href="/" class="btn btn-ghost normal-case text-xl" on:click={() => (showWarning = false)}
			>NEXT-JOBS</a
		>
		<div>
			{#if $isLoggedInStore}
			...
			{:else}
				...
				<a href="/users/login/" class="btn" on:click={() => (showWarning = false)}>Login</a>
			{/if}
		</div>
	</nav>
	{#if showWarning}
		<div class="alert alert-warning shadow-lg mt-5">
			<svg
				...
				/></svg
			>
			<span class="absolute ml-8">Please sign up first!</span>
		</div>
	{/if}
</header>
<slot />
